...
class App extends React.Component {
    ...
    componentDidMount() {
        this.handleSubmitValue('je te le donne');
    }
    ...
    handleSubmitValue = async (searchTerm)=> {
        const response = await youtube.get('search',  { 
            params: {
                part: 'snippet', 
                maxResults: 10,
                key:'AIzaSyDTaaIftzK2dQlXH6qTYzCMm9hPHuqIVu8',
                q: searchTerm,
    }
           ...
           <Grid justify="center" container spacing={10}>
               <Grid item xs={12}>
                   <Grid container spacing={10}>
                       <Grid item xs={12}>
                        <SearchBar onFormSubmit={ this.handleSubmitValue}/>
                       </Grid>
                       <Grid item xs={8}>
                        < VideoDetail video = {
                            selectedVideo
